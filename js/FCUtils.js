...
fc.utils.isAlphanumericKeyPlusAndMaxLength = function (event, sPlusThese, nMaxChars) {
    var sValidChars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" + sPlusThese;
    return fc.utils.isValidKeyAndMaxLength(event, sValidChars, nMaxChars);
}
...
fc.utils.getDisplayTimestamp = function (bUtc, bMillis) {
    // Return a Local (default) or UTC timestamp as a string,
    // with or without (default) milliseconds

    // Default Syntax
    bUtc = (typeof bUtc !== 'undefined') ? bUtc : false; // Default to false
    bMillis = (typeof bMillis !== 'undefined') ? bMillis : false; // Default to false


    var timestamp = new Date();

    var nHours, nMinutes, nSeconds, nMillis = 0;

    if (bUtc) {
        nHours = timestamp.getUTCHours();
        nMinutes = timestamp.getUTCMinutes();
        nSeconds = timestamp.getUTCSeconds();
        if (bMillis) {
            nMillis = timestamp.getUTCMilliseconds();
        }
    }
    else {
        nHours = timestamp.getHours();
        nMinutes = timestamp.getMinutes();
        nSeconds = timestamp.getSeconds();
        if (bMillis) {
            nMillis = timestamp.getMilliseconds();
        }
    }
    ...
    var sMillis = "";
    if (bMillis) {
        sMillis = fc.utils.prePad(nMillis.toString(10), "0", 3);
    }

    var sTimestamp = sHours + ":" + sMinutes + ":" + sSeconds;
    if (bMillis) {
        sTimestamp = sTimestamp + "." + sMillis;
    }
...
fc.utils.decodeURIComponent = function (s) {
    var pl = /\+/g;  // Regex for replacing addition symbol with a space	
    return decodeURIComponent(s.replace(pl, " "));
}

fc.utils.buildQueryStringObject = function (obj) {
    var match;
    var search = /([^&=]+)=?([^&]*)/g;
    var query = window.location.search.substring(1);

    while (match = search.exec(query)) {
        var property = fc.utils.decodeURIComponent(match[1]);
        var value = fc.utils.decodeURIComponent(match[2]);
        obj[property] = value;
    }
}

// Return true if the passed string is one that might be construed as 'false'
fc.utils.isFalseString = function (s) {
    var sLowerCase = s.toLowerCase();
    return (sLowerCase == "0" || sLowerCase == "false" || sLowerCase == "n");
}
